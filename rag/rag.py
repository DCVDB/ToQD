...
class Retriever:
    ...
    def setup_retriever(self):
        ...
        self.model, self.tokenizer, _ = contriever.load_retriever(self.args.model_name_or_path,cache_dir=self.args.cache_dir)
        self.model.eval()
        self.model = self.model.cuda()
        if not self.args.no_fp16:
            self.model = self.model.half()

        self.index = index.Indexer(self.args.projection_size, self.args.n_subquantizers, self.args.n_bits)
        ...
        self.passages = data.load_passages(self.args.passages)
    ...
    def setup_retriever_demo(self, model_name_or_path, passages, passages_embeddings, n_docs=5, save_or_load_index=False,cache_dir=None):
        print(f"Loading model from: {model_name_or_path}")
        self.model, self.tokenizer, _ = contriever.load_retriever(model_name_or_path,cache_dir=cache_dir)
        self.model.eval()
        self.model = self.model.cuda()

        self.index = index.Indexer(768, 0, 8)
        ...
        # load passages
        print("loading passages")
        self.passages = data.load_passages(passages)
        self.passage_id_map = {x["id"]: x for x in self.passages}
        print("passages have been loaded")
...
retriever.setup_retriever_demo("facebook/contriever-msmarco", "./db/enwiki_2020_intro_only/enwiki_2020_dec_intro_only.jsonl", "./db/enwiki_2020_intro_only/enwiki_dec_2020_contriever_intro/*",  n_docs=5, save_or_load_index=False,cache_dir="./repo")
retrieved_documents = retriever.search_document_demo(query_3, 10)
print(retrieved_documents)
retrieved_document = retriever.search_document_demo("Hello,world!", 10)
...
retriever = Retriever(args)
retriever.setup_retriever()
print(retriever.search_document("Hello, world!",top_n=10))
"""