...
def load_retriever(model_path, pooling="average", random_init=False, cache_dir=None):
    ...
    if os.path.exists(path):
        ...
        tokenizer = utils.load_hf(transformers.AutoTokenizer, retriever_model_id, cache_dir)
        cfg = utils.load_hf(transformers.AutoConfig, retriever_model_id, cache_dir)
        ...
        if "xlm" in retriever_model_id:
            ...
            model_class = Contriever
        cfg = utils.load_hf(transformers.AutoConfig, model_path, cache_dir)
        tokenizer = utils.load_hf(transformers.AutoTokenizer, model_path, cache_dir)
        retriever = utils.load_hf(model_class, model_path, cache_dir)
